...
get_csv = [f for f in listdir("mined data/Cap-2 Student Info/") if isfile(join("mined data/Cap-2 Student Info/", f))]
...
data=pd.read_csv("mined data/Cap-2 Student Info/student0201-1002.csv")
...
for file in get_csv:
    data2=pd.read_csv("mined data/Cap-2 Student Info/{}".format(file))
    data=pd.concat([data[new_columns],data2[new_columns]], ignore_index=True)
        
    
data.to_csv("final data/Cap-2 Student Info.csv")